...
$statement = query('SELECT c.ID AS category_id,c.name AS category_name,f.name AS forum_name,f.ID AS forum_id FROM forum AS f LEFT JOIN category AS c ON c.ID=f.category_ID');
$statement->bind_result($category_id,$category_name, $forum_name, $forum_id);
